...
exports.createInitialGame = createInitialGame;
exports.checkWinner = checkWinner;
exports.placeStone = placeStone;
exports.useSkill = useSkill;
const skills_1 = require("./skills");
function createInitialGame() {
    const board = Array.from({ length: 5 }, () => Array.from({ length: 5 }, () => ({
        type: 'empty',
        hp: 1,
        statusEffects: []
    })));
...
function checkWinner(board) {
    ...
    const WIN_COUNT = 4;
    const playerSymbol = (player) => (player === 'A' ? 'circle' : 'cross');
...
function useSkill(state, skillId, target, customPayload) {
    if (state.winner)
        return state;
